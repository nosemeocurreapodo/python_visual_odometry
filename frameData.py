...
class frameData:
  ...
  def setInvDepth(self, _invDepth, _invDepthVar):
    for lvl in range(0, params.MAX_LEVELS):
      width = int(params.IMAGE_WIDTH/(2**lvl))
      height = int(params.IMAGE_HEIGHT/(2**lvl))
      self.invDepth[lvl] = cv2.resize(_invDepth, (width, height), interpolation=cv2.INTER_AREA)
      self.invDepthVar[lvl] = cv2.resize(_invDepthVar, (width, height), interpolation=cv2.INTER_AREA)
